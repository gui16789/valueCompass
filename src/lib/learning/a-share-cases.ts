export type AShareCaseStudy = {
  ...
  sourceHints: string[];
  researchDossier?: AShareResearchDossier;
  valuationWalkthrough?: AShareValuationWalkthrough;
};

export type AShareResearchDossier = {
  boundaryNote: string;
  sections: Array<{
    title: string;
    summary: string;
...
const aShareCaseStudies: AShareCaseStudy[] = [
  ...
  },
  {
    ...
    researchDossier: {
      boundaryNote:
        "这份研究档案用于把公司研究拆成事实、假设、待验证项和投资纪律，不构成对华友钴业的买入、卖出或持有建议。所有价格和估值条件都必须由用户更新最新财报、股本、市场价格和商品价格后重新计算。",
      sections: [
        {
          title: "1. 公司一句话：它靠什么赚钱？",
          summary:
            "华友钴业靠镍、钴、锂、铜等资源和新能源材料一体化链条赚钱，利润来自资源价格、冶炼加工价差、正极材料/前驱体出货、客户订单和产能利用率共同作用。",
          items: [
            "不是单纯钴公司，也不是单纯正极材料公司，而是资源 + 冶炼 + 前驱体 + 正极材料 + 回收的一体化平台。",
            "收入大头来自新能源电池材料及原料、有色金属和贸易及其他；毛利贡献更依赖镍、钴、锂、铜价格和材料价差。",
            "研究时先问：当金属价格下行时，公司是否还能靠成本、客户和一体化维持利润。"
          ]
        },
        {
          title: "2. 能力圈判断：我是否真的理解？边界在哪里？",
          summary:
            "如果只能解释新能源车需求，而不能解释镍钴锂价格、资源项目、材料价差、资本开支和海外风险，就还没有进入能力圈。",
          items: [
            "必须能说清：镍、钴、锂、铜各自如何影响利润，正极材料和前驱体如何定价。",
            "必须能看懂：经营现金流、资本开支、在建工程、资产负债率和存货变化。",
            "能力圈边界：海外矿山政策、商品价格短期走势、技术路线切换和客户订单兑现，都不能假装完全可预测。"
          ]
        },
        {
          title: "3. 商业模式：客户 / 产品 / 渠道 / 成本结构 / 盈利驱动",
          summary:
            "商业模式核心是上控资源、中提能力、下拓客户：资源保障降低原料约束，材料加工连接电池和整车客户，利润随价格、销量、成本和开工率波动。",
          items: [
            "客户：电池厂、正极材料客户、消费电子和新能源车产业链客户，部分订单来自国际客户和长单协议。",
            "产品：镍产品、钴产品、锂产品、铜产品、三元前驱体、正极材料和回收相关业务。",
            "渠道：以直销和产业链长单为主，客户质量和订单稳定性比传统渠道铺货更重要。",
            "成本结构：原料成本、能源、折旧、人工、财务费用和海外项目爬坡成本，决定周期底部抗压能力。",
            "盈利驱动：金属价格、产品销量、毛利率、产能利用率、资源自供比例和资本开支效率。"
          ]
        },
        {
          title: "4. 护城河：品牌 / 成本 / 转换成本 / 网络效应 / 规模经济 / 稀缺资源",
          summary:
            "华友的护城河不主要来自消费品牌，而来自资源布局、一体化成本、规模制造、客户认证和项目执行能力。",
          items: [
            "成本优势：资源端、冶炼端和材料端一体化如果能降低综合成本，才是真护城河。",
            "转换成本：动力电池和材料供应链验证周期较长，进入核心客户体系后存在一定切换成本。",
            "规模经济：大规模采购、生产和项目管理有助于摊薄成本，但扩产过快也可能放大周期风险。",
            "稀缺资源：海外镍、钴、锂资源布局有价值，但也伴随资源国政策、ESG、汇率和地缘风险。",
            "网络效应：不明显，不能用互联网式网络效应给估值溢价。"
          ]
        },
        {
          title: "5. 管理层：诚信 / 资本配置 / 股东导向 / 历史兑现承诺",
          summary:
            "管理层研究重点不是口号，而是扩产纪律、负债控制、项目兑现、分红回购和信息披露的一致性。",
          items: [
            "诚信：持续检查重大项目、产能、客户订单和利润指引是否与后续结果一致。",
            "资本配置：重点看重资产扩张是否创造 ROIC，而不是只扩大收入规模。",
            "股东导向：关注分红、回购、股权激励、可转债转股和股份摊薄对长期股东的影响。",
            "历史兑现：把过往海外项目爬坡、产能释放、成本下降和现金流改善放进跟踪表。"
          ]
        },
        {
          title: "6. 财务质量：收入质量、毛利率、净利率、ROIC、现金转化率、负债",
          summary:
            "2025 年利润创高，但经营现金流低于净利润且资本开支较重，因此财务质量要重点看现金转化和资产负债表，而不能只看利润增速。",
          items: [
            "收入质量：拆分价格上涨、销量增长和贸易收入，避免把价格周期误读为经营能力。",
            "毛利率 / 净利率：按镍、钴、锂、铜、前驱体、正极材料分产品跟踪，而不是只看综合毛利率。",
            "ROIC：重资产公司要看新增投入资本能否产生高于资本成本的回报。",
            "现金转化率：2025 年经营现金流约 40.12 亿元，低于归母净利润 61.10 亿元，需要解释预付原料、存货和应收变化。",
            "负债：2025 年资产负债率约 61.85%，必须检查债务期限、利率、项目融资和下行周期偿债压力。"
          ]
        },
        {
          title: "7. 估值模型：保守 / 中性 / 乐观三档内在价值区间",
          summary:
            "主模型用周期制造中枢利润法，预测模型用研报利润假设做 PE 推演，再用 PB、自由现金流和反向 DCF 交叉验证。",
          items: [
            "历史中枢利润法：悲观约 13-21 元 / 股，中性约 24-35 元 / 股，乐观约 38-59 元 / 股，偏重保守周期视角。",
            "研报预测 PE 法：悲观约 40 元 / 股，中性约 75-85 元 / 股，乐观约 116-165 元 / 股，偏重产能兑现和景气延续视角。",
            "两套模型差异很大，说明结论高度依赖对金属价格、产能释放和利润中枢的判断。",
            "不能把乐观区间叫目标价；它只是某组假设下的数学结果。"
          ]
        },
        {
          title: "8. 安全边际：当前价格相对内在价值折价",
          summary:
            "安全边际不是“上涨空间”，而是参考价格相对保守价值是否足够便宜，以及在关键假设错了时损失是否可控。",
          items: [
            "若使用 2026-04-30 约 66 元作为参考价，它高于保守周期模型的乐观上沿 59 元，也高于研报悲观情景约 40 元。",
            "若只相信保守周期模型，66 元没有明显安全边际；若相信研报中性模型，才可能出现估值折价。",
            "因此安全边际来自对中性/乐观假设的证据确认，而不是来自悲观情景。",
            "纪律写法：安全边际 = 经自己复核后的保守内在价值 - 当前价格，而不是当前价格到乐观情景的差额。"
          ]
        },
        {
          title: "9. 关键假设：3-5 个必须持续验证的假设",
          summary:
            "华友的关键假设应压缩到少数真正影响价值的变量，避免每天被股价和新闻牵着走。",
          items: [
            "金属价格：镍、钴、锂、铜是否维持在支持预测利润的区间。",
            "产能释放：Pomalaa、津巴布韦锂项目、匈牙利正极等是否按计划爬坡。",
            "客户和出货：正极材料、前驱体、镍产品、锂产品出货是否兑现研报假设。",
            "现金流：经营现金流能否跟上利润，资本开支是否开始回落或产生回报。",
            "资产负债表：负债率、财务费用、存货和在建工程是否处于可控范围。"
          ]
        },
        {
          title: "10. 风险清单：最多 3 个真正重要的风险",
          summary:
            "风险清单要少而硬，优先记录会推翻估值模型的风险。",
          items: [
            "商品价格下行：镍、钴、锂价格低于模型假设，利润中枢和估值倍数同时下修。",
            "重资产扩张失败：项目延期、成本超支、产能利用率不足，导致折旧、利息和现金流压力放大。",
            "海外与政策风险：印尼、津巴布韦等资源项目面临政策、税费、汇率、ESG、地缘和合规不确定性。"
          ]
        },
        {
          title: "11. 买入条件：什么价格、什么事实成立才买",
          summary:
            "这里记录的是进入投资委员会讨论的前置条件，不是系统建议当前买入。价格条件必须绑定已复核的事实条件。",
          items: [
            "价格条件：参考价格需要低于自己复核后的保守价值，或相对中性价值有足够折扣；不能因为乐观情景空间大就降低安全边际。",
            "事实条件：2026 年利润预测需要被季度利润、出货、毛利率和现金流同时验证。",
            "周期条件：镍、钴、锂价格不能明显跌破模型关键假设，或公司必须证明成本优势足以抵消价格下行。",
            "资产负债表条件：负债率、现金流、资本开支和项目融资不能恶化到威胁下行周期生存质量。",
            "认知条件：必须能写出反方观点，并说明什么事实会推翻自己的买入理由。"
          ]
        },
        {
          title: "12. 卖出条件：什么情况必须重新审查或卖出",
          summary:
            "这里记录的是重新审查触发器和纪律边界，不是系统建议当前卖出。真正重要的是知道什么事实会推翻原始判断。",
          items: [
            "估值触发：价格显著高于自己复核后的中性价值，且乐观情景需要过多假设同时成立时，必须重新审查预期收益。",
            "基本面触发：连续季度出现出货不及预期、毛利率下滑、经营现金流恶化或库存压力上升。",
            "资本配置触发：重资产项目持续超支、ROIC 低于资本成本，或频繁融资摊薄但现金流没有改善。",
            "风险触发：资源国政策、重大安全环保、客户流失或技术路线变化，使原来的护城河假设失效。",
            "认知触发：如果已经无法解释利润来源和关键变量，应该先退出能力圈判断，重新做研究。"
          ]
        }
      ]
    },
